 ...
 *
 * Icon rules: viewBox 0 0 100 100, stroke-only paths (no fills), stroke-width
 * 5, currentColor, hand-drawn line-art feel, ~8-unit margin.
 */
...
// -- the library ------------------------------------------------------------
const ICONS: Record<AssetTag, { paths: string[]; synonyms: string[] }> = {
  person: {
    paths: [circle(50, 30, 13), "M 24 84 Q 24 54 50 54 Q 76 54 76 84"],
  ...
  gear: {
    paths: [circle(50, 50, 15), circle(50, 50, 26), ...rays(50, 50, 26, 34, 8, Math.PI / 8)],
...
for (const tag of ASSET_TAGS) {
  ...
  manifest[tag] = { file: `${tag}.svg`, synonyms };
}
writeFileSync(
